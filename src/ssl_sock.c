...
static int ssl_sock_load_ocsp(SSL_CTX *ctx, const struct cert_key_and_chain *ckch, char *chain_file)
{
	X509 *x, *issuer;
	...
	STACK_OF(X509) *ovch = ckch->chain ? ckch->chain : (chain_file ? ssl_get0_chain_file(chain_file) : NULL);
	...
	for (i = 0; i < sk_X509_num(ovch); i++) {
		X509 *ti = sk_X509_value(ovch, i);
				      ...
				      char *chain_file, SSL_CTX *ctx, char **err)
{
	...
	if (ckch->ocsp_response) {
		if (ssl_sock_load_ocsp(ctx, ckch, chain_file) < 0) {
