...
static THREAD_LOCAL int volatile evports_panic_errno = 0;
...
{
	if (events == 0) {
		if (port_dissociate(evports_fd[tid], PORT_SOURCE_FD, fd) != 0) {
		...
		if (port_associate(evports_fd[tid], PORT_SOURCE_FD, fd, events,
		    NULL) != 0) {
			evports_panic_errno = errno;
			send_log(NULL, LOG_EMERG,
